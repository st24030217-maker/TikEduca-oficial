...
window.submitTikeduca = async function() {
 ...
 const err = document.getElementById("formError");
 if (err) {
 err.textContent = " Error de conexión con el servidor. Por favor, reintenta en unos momentos.";
 err.classList.remove("hidden");
 setTimeout(() => {
 err.classList.add("hidden");
 setTimeout(() => { err.textContent = " Por favor completa todos los campos obligatorios"; }, 300);
...
window.submitPayment = async function() {
 ...
 <p class="text-red-400 text-xs font-bold mb-1 font-orbitron tracking-wider"> ERROR DE GUARDADO AUTOMÁTICO</p>
 <p class="text-gray-300 text-[11px] leading-relaxed font-space">
